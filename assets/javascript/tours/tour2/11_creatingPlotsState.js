const creatingPlotsState = `;;; SETUP PROCEDURE
patches-own [p_ext]
extensions[fetch import-a]
to setup
  clear-all
  import-a:pcolors fetch:url (word "https://raw.githubusercontent.com/SantaFeInstitute/ABMA/master/ch4/ch4_veg.png")
  reset-ticks
  create-turtles 20 [
    set size 2
    set shape "turtle"
...
to go
  ...
  ask turtles [
    ;right random 360
    ;forward 1
    if random-float 1 <= pop_growth [reproduce]
    if random-float 1 <= p_ext [die]
  ]
end

to reproduce
  let empty-patches patches in-radius move-radius with [count turtles-here = 0 AND pcolor != white]
  if any? empty-patches [
    hatch 1 [
      set color color + 0.1
      ifelse foresight? = true
      [
